...
// JSONSchemaVersion is the JSON Schema version.
...
type Copyable[T any] interface {
	...
	// Implementers should return a map that is compatible for use as a JSON document.
	Map() map[string]any
}

// JSONSchemaGeneric represents a JSON Schema object type.
//
...
func (js *JSONSchemaGeneric[T]) DeepCopy() *JSONSchemaGeneric[T] {
	...
	if l := js.Properties.Len(); l > 0 {
		newJs.Properties = orderedmap.New[string, T](l)
		for p := js.Properties.Oldest(); p != nil; p = p.Next() {
			newJs.Properties.Set(p.Key, p.Value.DeepCopy())
		}
	}

	if l := js.PatternProperties.Len(); l > 0 {
		newJs.PatternProperties = orderedmap.New[string, T](l)
		for p := js.PatternProperties.Oldest(); p != nil; p = p.Next() {
			newJs.PatternProperties.Set(p.Key, p.Value.DeepCopy())
		}
	}

	if l := len(js.Definitions); l > 0 {
		newJs.Definitions = make(map[string]T, l)
...
// JSONSchema is a plain spec-only flavour – just an alias.
...
type JSONSchemaRAML struct {
	...
	Annotations                        *orderedmap.OrderedMap[string, any]             `yaml:"x-annotations,omitempty"       json:"x-annotations,omitempty"`
	FacetDefinitions                   *orderedmap.OrderedMap[string, *JSONSchemaRAML] `yaml:"x-facet-definitions,omitempty" json:"x-facet-definitions,omitempty"`
	FacetData                          *orderedmap.OrderedMap[string, any]             `yaml:"x-facet-data,omitempty"        json:"x-facet-data,omitempty"`
}
...
func (js *JSONSchemaRAML) ShallowCopy() *JSONSchemaRAML {
	...
	if l := js.Annotations.Len(); l > 0 {
		newJs.Annotations = orderedmap.New[string, any](l)
		for p := js.Annotations.Oldest(); p != nil; p = p.Next() {
			newJs.Annotations.Set(p.Key, p.Value)
		}
	}
	if l := js.FacetDefinitions.Len(); l > 0 {
		newJs.FacetDefinitions = orderedmap.New[string, *JSONSchemaRAML](l)
		for p := js.FacetDefinitions.Oldest(); p != nil; p = p.Next() {
			newJs.FacetDefinitions.Set(p.Key, p.Value)
		}
	}
	if l := js.FacetData.Len(); l > 0 {
		newJs.FacetData = orderedmap.New[string, any](l)
...
func (js *JSONSchemaRAML) Map() map[string]any {
	...
	if l := js.Annotations.Len(); l > 0 {
		annotations := make(map[string]any, l)
		for p := js.Annotations.Oldest(); p != nil; p = p.Next() {
	...
	if l := js.FacetDefinitions.Len(); l > 0 {
		facetDefs := make(map[string]any, l)
		for p := js.FacetDefinitions.Oldest(); p != nil; p = p.Next() {
	...
	if l := js.FacetData.Len(); l > 0 {
		facetData := make(map[string]any, l)
		for p := js.FacetData.Oldest(); p != nil; p = p.Next() {
...
func JSONSchemaWrapper(
	c *JSONSchemaConverter[*JSONSchemaRAML],
	core *JSONSchemaGeneric[*JSONSchemaRAML],
	b *raml.BaseShape,
) *JSONSchemaRAML {
	if core == nil {
		return nil
	...
	if n := b.CustomDomainProperties.Len(); n > 0 {
		...
		for p := b.CustomDomainProperties.Oldest(); p != nil; p = p.Next() {
			m.Set(p.Key, p.Value.Extension.Value.Raw)
		}
		w.Annotations = m
	}
